...
class DataBase(object):
    ...
    def __init__(self, args) -> None:
        ...
        # new
        self.kg_lines = self.path + "/numTrainLines.txt"
        # -----
        ...
        # self.inter_train_data, inter_val_data = self._train_validation_split(inter_data)

        (
            self.inter_train_data,
            inter_test_data,
            inter_val_data,
            self.inter_val_e,
            self.inter_val_neg,
        ...
        if self.no_test == False:
            (
                self.inter_train_data,
                inter_test_data,
                inter_val_data,
            ) = self._train_test_split(inter_data)
        else:
            self.inter_train_data, inter_val_data = self._train_validation_split(
                inter_data
            )
    ...
    def _train_validation_split(self, inter_data: np.array) -> tuple:
        """Splitting interaction data into training AND validation only"""
        ...
        inter_train_data, inter_val_data = train_test_split(
            inter_data, test_size=self.val_size, random_state=2021
        )
        ...
        return inter_train_data, inter_val_data
    ...
    def _log_data_info(self) -> None:
        ...
        logger.debug(
            "[n_train_inter, n_test_inter, n_val_inter]=[%d, %d, %d]"
            % (self.n_train_inter, self.n_test_inter, self.n_val_inter)
        )

        logger.info("Knowledge graph")
        logger.debug("[n_triple]=[%d]" % self.n_triple)
        logger.debug(
            "[n_entity, n_relation and n_attr]=[%d, %d, %d]"
            % (self.n_entity, self.n_relation, self.n_attr)
        )
        logger.info("Detection model")
        logger.debug("[training epoch]=[%d]" % self.args.epoch)
        logger.debug(
            "[batch_size_gnn and batch_size_kg]=[%d, %d]"
            % (self.batch_size_gnn, self.batch_size_kg)
        )
    ...
    def _load_kg_stat(self) -> tuple:
        ...
        # new - changed file read
        with open(self.kg_lines, "r") as f:
    ...
    def _generate_train_inter_batch(self) -> tuple:
        ...
        if self.batch_size_gnn <= self.exist_entity_size:
            ...
            e_batch = np.array(
                [rd.choice(self.exist_entity) for _ in range(self.batch_size_gnn)]
            )
    ...
    def _load_gt(self) -> dict:
        ...
        for l in lines:
            ...
            e1, e2 = inter[0], inter[1]
            if e1 not in gt_dict:
                # situation: e1 first appears
                gt_dict[e1] = [e2]
            else:
                # situation: e1 has appeared in previous behaviors
                exist_inters = gt_dict[e1]
                exist_inters.append(e2)
