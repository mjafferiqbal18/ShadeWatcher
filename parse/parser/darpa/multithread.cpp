...
void MergeGraph(KG *infotbl_thread, KG *infotbl)
{
    ...
    for (auto it : infotbl_thread->nodeHashToUuid)
    {
        hash_t n_hash = it.first;
        std::string n_uuid = it.second;
        infotbl->nodeHashToUuid[n_hash] = n_uuid;
    }

    for (auto it : infotbl_thread->maliciousEdgeTable)
    {
        hash_t e_id = it.first;
        KGEdge *e = it.second;
...
void MultithreadConstruction(std::string darpa_file, KG *infotbl, Config &cfg)
{
    ...
    std::cout << "NUM EDGES BEFORE NR: " << infotbl->KGEdgeTable.size() << std::endl;
    ...
    std::cout << "NUM EDGES AFTER NR: " << infotbl->KGEdgeTable.size() << std::endl;
