...
// Map for listing children and parents
nodeRelationshipsMap nodeRelationshipsMapObj;
...
KG::KG(int _audit_source)
{
        ...
        // std::cout << "Object interaction map initial state" << std::endl;
        // std::cout << "ObjectInteractionTable size: " << ObjectInteractionTable.size() << std::endl;
        // for (auto elem: ObjectInteractionTable) {
...
void KG::PrintFd()
{
        for (auto it : ProcFdMap)
        {
                ...
                std::cout << "Proc: " << p->pid << " " << p->exe << " " << p->args << " " << p_id << " has opened:" << std::endl;
...
void KG::FindAllObjectInteractions()
{
        for (const auto &it : KGEdgeTable)
        {
                ...
                nodeRelationshipsMapObj[parent].children.push_back(edge);

                nodeRelationshipsMapObj[child].parents.push_back(edge);
        }

        for (auto &it : KGNodeTable)
        {
                if (SearchNodeType(it.first) == NodeType_t::File)
                { // check File
...
void KG::InsertMaliciousEdge(hash_t e_id, KGEdge *edge)
{
        while (1)
        {
                auto _it = maliciousEdgeTable.find(e_id);
                if (_it != maliciousEdgeTable.end())
                {
                        e_id++;
                        edge->e_id = e_id;
                }
        ...
        maliciousEdgeTable[e_id] = edge;
}

void KG::InsertNodeHashToUuidPair(hash_t n_hash, std::string n_uuid)
{
        nodeHashToUuid[n_hash] = n_uuid;
}